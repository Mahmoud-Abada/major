...
export default function AttendanceContent() {
  ...
  const handleViewDetails = (
    item: AttendanceRecord | AttendanceEvent | StudentAttendance,
  ) => {
    if ("studentId" in item && "status" in item) {
      setSelectedRecord(item as AttendanceRecord);
    } else if (
      "totalStudents" in item &&
      "attendanceRate" in item &&
      "events" in item
    ) {
      ...
      cell: ({ row }: { row: { original: AttendanceEvent } }) => (
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm">
            {row.original.presentCount + row.original.lateCount}/
            {row.original.totalStudents}
          </span>
          <Badge
            variant={
              row.original.attendanceRate >= 80 ? "default" : "destructive"
            }
          >
  ...
  const recordsColumns = [
    ...
    },
    {
      ...
      header: "Time In",
      cell: ({ row }: { row: { original: AttendanceRecord } }) => (
        <span className="text-sm font-mono">{row.original.timeIn || "-"}</span>
      ),
    },
    {
      accessorKey: "timeOut",
      header: "Time Out",
      cell: ({ row }: { row: { original: AttendanceRecord } }) => (
        <span className="text-sm font-mono">{row.original.timeOut || "-"}</span>
      ),
    },
    {
        ...
        <div className="text-sm">
          {row.original.lastAttendance ? (
            <div>
              <div>
                {new Date(
                  row.original.lastAttendance.date,
                ).toLocaleDateString()}
              </div>
