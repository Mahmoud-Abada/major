...
export function AttendanceStats({
  ...
  const todayAttendanceRate =
    todayRecords.length > 0
      ? ((todayPresent + todayLate) / todayRecords.length) * 100
      : 0;
  ...
  const overallAttendanceRate =
    records.length > 0
      ? ((overallPresent + overallLate) / records.length) * 100
      : 0;
  ...
  const weeklyAttendanceRate =
    weeklyRecords.length > 0 ? (weeklyPresent / weeklyRecords.length) * 100 : 0;

  // Class distribution
  const classDistribution = classroomAttendance.reduce(
    (acc, classroom) => {
        ...
        <CardContent className="space-y-3 md:space-y-4">
          ...
          <div>
            <div className="flex justify-between text-xs md:text-sm mb-2">
              ...
              <span className="font-medium">
                {weeklyAttendanceRate.toFixed(1)}%
              </span>
            </div>
