...
export function StudentsView() {
    const [studentSummaries] = useState<StudentPostSummary[]>(mockStudentSummaries);
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedStudent, setSelectedStudent] = useState<StudentPostSummary | null>(null);
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <Progress value={(student.averageGrade / 20) * 100} className="h-2" />
                                ...
                                <div>
                                    ...
                                    <div className="space-y-1">
                                        {student.upcomingDeadlines.slice(0, 2).map((deadline) => (
                                            <div key={deadline.id} className="flex items-center justify-between text-xs p-2 bg-muted/30 rounded">
                                                <span className="line-clamp-1">{deadline.title}</span>
                                                <Badge variant="outline" className="text-xs">
                                                    {new Date(deadline.dueDate).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}
                                                </Badge>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <CardTitle className="text-xl">{selectedStudent.studentName}</CardTitle>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="text-center p-4 bg-blue-50 rounded-lg">
                                ...
                                <p className="text-sm text-muted-foreground">Publications Créées</p>
                            </div>
                            ...
                            <div className="text-center p-4 bg-green-50 rounded-lg">
                                ...
                                <p className="text-sm text-muted-foreground">Commentaires Postés</p>
                            </div>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        <Progress value={(selectedStudent.averageGrade / 20) * 100} className="h-3" />
                                        <p className="text-sm text-muted-foreground mt-2">
                                            Niveau: <span className={`font-medium ${getGradeColor(selectedStudent.averageGrade)}`}>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        <div className="flex justify-between text-sm mb-2">
                                            <span>Score d'Engagement</span>
                                            <span className={`font-medium ${getEngagementColor(selectedStudent.engagementScore)}`}>
                                                {selectedStudent.engagementScore.toFixed(1)}%
                                            </span>
                                        </div>
                                        <Progress value={selectedStudent.engagementScore} className="h-3" />
                                        <p className="text-sm text-muted-foreground mt-2">
                                            Participation très{" "}
                                            {selectedStudent.engagementScore >= 80 ? "active" :
                                                selectedStudent.engagementScore >= 60 ? "bonne" :
                                                    selectedStudent.engagementScore >= 40 ? "modérée" : "faible"}
                                        </p>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        {selectedStudent.upcomingDeadlines.length > 0 && (
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-base flex items-center gap-2">
                                        <RiCalendarLine className="h-4 w-4" />
                                        Échéances à Venir ({selectedStudent.upcomingDeadlines.length})
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="space-y-3">
                                        {selectedStudent.upcomingDeadlines.map((deadline) => (
                                            <div key={deadline.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <p className="text-xs text-muted-foreground mt-1">
                                                        {Math.ceil((new Date(deadline.dueDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} jours
                                                    </p>
                                                </div>
                                            </div>
                                        ))}
