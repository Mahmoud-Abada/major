...
export function ClassesView() {
  ...
  const [selectedClass, setSelectedClass] = useState<ClassPostOverview | null>(null);
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className={`font-medium ${getEngagementColor(classOverview.averageEngagement)}`}>
                    {classOverview.averageEngagement.toFixed(1)}%
                  </span>
                </div>
                <Progress value={classOverview.averageEngagement} className="h-2" />
              </div>
              ...
              {classOverview.topContributors.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {classOverview.topContributors.slice(0, 2).map((contributor) => (
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className={`font-medium ${getEngagementColor(selectedClass.averageEngagement)}`}>
                          {selectedClass.averageEngagement.toFixed(1)}%
                        </span>
                      </div>
                      <Progress value={selectedClass.averageEngagement} className="h-3" />
                    </div>
                    
                    <div className="text-sm text-muted-foreground">
                      <p>
                        Cette classe montre un niveau d'engagement{" "}
                        {selectedClass.averageEngagement >= 80 ? "excellent" :
                         selectedClass.averageEngagement >= 60 ? "bon" :
                         selectedClass.averageEngagement >= 40 ? "moyen" : "faible"}.
                      </p>
                    </div>
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {selectedClass.topContributors.map((contributor, index) => (
                      <div key={contributor.userId} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              <span>•</span>
                              <span>{contributor.commentsCount} commentaires</span>
                            </div>
                          </div>
                        </div>
                      </div>
