...
export default function MarksContent() {
  ...
  const [studentMarks, setStudentMarks] =
    useState<StudentMark[]>(mockStudentMarks);
  ...
  const [selectedStudentMark, setSelectedStudentMark] =
    useState<StudentMark | null>(null);
  const [selectedGroupMark, setSelectedGroupMark] = useState<GroupMark | null>(
    null,
  );
  ...
  const handleUpdateMark = (updatedMark: Mark) => {
    setMarks(
      marks.map((mark) => (mark.id === updatedMark.id ? updatedMark : mark)),
    );
  ...
  const studentMarksColumns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: StudentMark } }) => {
        const percentage =
          (row.original.obtainedMarks / row.original.totalMarks) * 100;
        return (
          <div className="flex items-center gap-2">
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: GroupMark } }) => (
        <span className="font-mono">
          {row.original.averageMarks.toFixed(1)}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }: { row: { original: GroupMark } }) => (
        <div className="flex items-center gap-2">
          ...
          <Badge variant="default">
            {(
              (row.original.passCount / row.original.totalStudents) *
              100
            ).toFixed(1)}
            %
          </Badge>
      ...
      cell: ({ row }: { row: { original: GroupMark } }) => (
        <div className="flex items-center gap-2">
          ...
          <Badge variant="destructive">
            {(
              (row.original.failCount / row.original.totalStudents) *
              100
            ).toFixed(1)}
            %
          </Badge>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage student marks, exams, and assessments
            </p>
          </div>
          <div className="flex items-center gap-2">
