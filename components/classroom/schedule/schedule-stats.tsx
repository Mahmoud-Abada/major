...
import type { ClassSchedule, Room, ScheduleEntry, TeacherSchedule } from "./types";
...
export function ScheduleStats({
    ...
    const subjectDistribution = entries.reduce((acc, entry) => {
    ...
    const teacherWorkload = teacherSchedules
        .map(teacher => ({
            ...
            utilizationRate: (teacher.totalWeeklyHours / teacher.maxWeeklyHours) * 100
        }))
                    ...
                    <CardContent className="p-3 md:p-4">
                        <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-lg md:text-2xl font-bold">{totalHours.toFixed(0)}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <CardContent className="space-y-3 md:space-y-4">
                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                        <div className="p-2 md:p-3 bg-blue-50 rounded-lg text-center">
                            <p className="text-base md:text-lg font-bold text-blue-600">{todayEntries.length}</p>
                            <p className="text-xs text-muted-foreground">{t('todaySessions') || "Today's Sessions"}</p>
                        </div>
                        <div className="p-2 md:p-3 bg-orange-50 rounded-lg text-center">
                            <p className="text-base md:text-lg font-bold text-orange-600">{ongoingEntries}</p>
                            <p className="text-xs text-muted-foreground">{t('ongoing') || 'Ongoing'}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                        <div className="p-2 md:p-3 bg-green-50 rounded-lg text-center">
                            <p className="text-base md:text-lg font-bold text-green-600">{completedEntries}</p>
                        ...
                        <div className="p-2 md:p-3 bg-red-50 rounded-lg text-center">
                            <p className="text-base md:text-lg font-bold text-red-600">{cancelledEntries}</p>
                ...
                <CardContent className="space-y-3 md:space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 md:gap-4 text-center">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">{t('teachers') || 'Teachers'}</p>
                        </div>
                        <div>
                            <p className="text-base md:text-lg font-bold">{totalRooms}</p>
                            <p className="text-xs text-muted-foreground">{t('rooms') || 'Rooms'}</p>
                        </div>
                    </div>
                </CardContent>
            </Card>
                        ...
                        {teacherWorkload.map((teacher, index) => (
                            <div key={teacher.teacherId} className="flex items-center justify-between">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                    ...
                                    <span className="text-xs md:text-sm truncate">{teacher.teacherName}</span>
                                </div>
                                <div className="flex items-center gap-1 md:gap-2 shrink-0">
                                    ...
                                    <Badge variant={
                                        teacher.utilizationRate >= 90 ? "destructive" :
                                            teacher.utilizationRate >= 70 ? "default" : "secondary"
                                    } className="text-xs">
                        ...
                        {roomUtilization.map((room, index) => (
                            <div key={room.roomId} className="flex items-center justify-between">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                    ...
                                    <span className="text-xs md:text-sm truncate">{room.roomName}</span>
                                </div>
                                <div className="flex items-center gap-1 md:gap-2 shrink-0">
                                    <span className="text-xs text-muted-foreground">
                                    ...
                                    <Badge variant={
                                        room.utilizationRate >= 80 ? "default" :
                                            room.utilizationRate >= 50 ? "secondary" : "outline"
                                    } className="text-xs">
            ...
            <Card className="shadow-sm">
                ...
                <CardContent className="space-y-3 md:space-y-4">
                    <div className="flex items-center justify-between">
                        <span className="text-xs md:text-sm flex-1 mr-2">{t('conflicts') || 'Conflicts'}</span>
                        <Badge
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant="default" className="text-xs shrink-0">
                            {totalEntries > 0 ? ((completedEntries / totalEntries) * 100).toFixed(1) : 0}%
                        </Badge>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card className="shadow-sm">
                ...
                <CardContent>
                    <div className="grid grid-cols-2 gap-3 md:gap-4 text-center">
                        <div>
                            <p className="text-base md:text-lg font-bold">{upcomingEntries}</p>
                            <p className="text-xs text-muted-foreground">{t('upcoming') || 'Upcoming'}</p>
                        </div>
                        <div>
                            <p className="text-base md:text-lg font-bold">
                                {Object.keys(dayDistribution).length}
                            </p>
                            <p className="text-xs text-muted-foreground">{t('activeDays') || 'Active Days'}</p>
                        </div>
                    </div>
                </CardContent>
            </Card>
