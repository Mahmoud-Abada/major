...
export default function ScheduleContent() {
    ...
    const [classSchedules, setClassSchedules] = useState<ClassSchedule[]>(mockClassSchedules);
    const [teacherSchedules, setTeacherSchedules] = useState<TeacherSchedule[]>(mockTeacherSchedules);
    ...
    const handleDeleteEntry = (entryId: string) => {
        setScheduleEntries(entries => entries.filter(entry => entry.id !== entryId));
    };

    const handleViewDetails = (item: ScheduleEntry | ClassSchedule | TeacherSchedule) => {
        if ('startTime' in item && 'endTime' in item) {
            setSelectedEntry(item as ScheduleEntry);
        } else if ('subjects' in item && 'schedule' in item && 'totalWeeklyHours' in item) {
                    ...
                    row.original.status === "completed" ? "default" :
                        row.original.status === "ongoing" ? "secondary" :
                            row.original.status === "cancelled" ? "destructive" :
                                row.original.status === "postponed" ? "outline" : "outline"
                }>
                    <div className="flex items-center gap-1">
                        {row.original.status === "completed" && <RiCheckLine size={14} />}
                        {row.original.status === "cancelled" && <RiAlarmWarningLine size={14} />}
                        {row.original.status}
                    </div>
                </Badge>
            ),
    ...
    const classColumns = [
        ...
        },
        {
            ...
            header: "Weekly Hours",
            cell: ({ row }: { row: { original: ClassSchedule } }) => (
                <span className="font-mono">{row.original.totalWeeklyHours.toFixed(1)}h</span>
            ),
        },
        {
    ...
    const teacherColumns = [
        ...
        },
        {
            ...
            header: "Utilization",
            cell: ({ row }: { row: { original: TeacherSchedule } }) => {
                const utilizationRate = (row.original.totalWeeklyHours / row.original.maxWeeklyHours) * 100;
                return (
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-muted-foreground">Manage class schedules, timetables, and room bookings</p>
                    </div>
                    <div className="flex items-center gap-2">
