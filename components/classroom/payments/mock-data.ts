...
const algerianParentNames = [
  "M. Benali Abderrahmane",
  "Mme. Benaissa Zohra",
  "M. Khelifi Mustapha",
  "Mme. Boumediene Khadija",
  "M. Hamidi Abdelkader",
  "Mme. Meziane Fatima",
  "M. Belkacem Salah",
  "Mme. Cherif Aicha",
  "M. Boukhalfa Omar",
  "Mme. Benabdallah Malika",
  "M. Mammeri Rachid",
  "Mme. Boudjelal Yamina",
  "M. Zidane Mohamed",
  "Mme. Benslimane Naima",
  "M. Boudiaf Youcef",
  "Mme. Hadj Soraya",
  "M. Mebarki Karim",
  "Mme. Benali Djamila",
...
for (let monthOffset = -6; monthOffset <= 3; monthOffset++) {
  ...
  for (let i = 0; i < paymentsPerMonth; i++) {
    const studentName =
      algerianStudentNames[
        Math.floor(Math.random() * algerianStudentNames.length)
      ];
    const parentName =
      algerianParentNames[
        Math.floor(Math.random() * algerianParentNames.length)
      ];
    const classInfo =
    ...
    const classMultiplier = classInfo.name.includes("Terminal")
      ? 1.3
      : classInfo.name.includes("3ème AS")
        ? 1.2
        : classInfo.name.includes("2ème AS")
          ? 1.1
          : classInfo.name.includes("1ère AS")
            ? 1.0
            : 0.8; // AM classes are cheaper

    const baseAmount = paymentTypeInfo.baseAmount * classMultiplier;
    const amount = Math.floor(
      baseAmount + (Math.random() - 0.5) * baseAmount * 0.2,
    ); // ±10% variation
    ...
    if (dueDate < now) {
      ...
      if (paymentProbability > 0.85) {
        ...
        paidAmount =
          Math.random() > 0.1
            ? amount
            : Math.floor(amount * (0.5 + Math.random() * 0.4)); // 10% partial payments
        paymentMethod =
    ...
    const lateFee =
      status === "overdue" ? Math.floor(amount * 0.05) : undefined; // 5% late fee

    // Generate discount (10% chance)
    const discount =
      Math.random() > 0.9
        ? Math.floor(amount * (0.05 + Math.random() * 0.15))
        : undefined; // 5-20% discount
    ...
    const installmentNumber =
      category !== "one_time" ? Math.floor(Math.random() * 10) + 1 : undefined;
...
const studentsWithPlans = Array.from(
  new Set(mockPayments.map((p) => p.studentId)),
).slice(0, 20);
...
studentsWithPlans.forEach((studentId, index) => {
  ...
  const planType = ["monthly", "quarterly", "semester"][
  ...
  const installmentCount =
    planType === "monthly" ? 10 : planType === "quarterly" ? 4 : 2;
  ...
  for (let i = 0; i < installmentCount; i++) {
    ...
    installments.push({
      ...
      lateFee:
        status === "overdue" ? Math.floor(installmentAmount * 0.05) : undefined,
    });
  }
  ...
  const planStatus =
    completedInstallments === installmentCount
      ? "completed"
...
uniqueStudents.forEach((student) => {
  const studentPayments = mockPayments.filter(
  ...
  if (studentPayments.length > 0) {
    ...
    const totalLateFees = studentPayments.reduce(
      (sum, p) => sum + (p.lateFee || 0),
      0,
    );
    const totalDiscounts = studentPayments.reduce(
    ...
    const lastPaymentDate =
      paymentHistory.length > 0 ? paymentHistory[0].paidDate : undefined;
    const nextPaymentDue =
      upcomingPayments.length > 0 ? upcomingPayments[0].dueDate : undefined;
...
overduePayments.forEach((payment, index) => {
  ...
  for (let i = 0; i < reminderCount; i++) {
    ...
    mockPaymentReminders.push({
      ...
      nextReminderDate:
        status === "sent" && i < 2
          ? new Date(reminderDate.getTime() + 7 * 24 * 60 * 60 * 1000)
              .toISOString()
              .split("T")[0]
          : undefined,
    });
