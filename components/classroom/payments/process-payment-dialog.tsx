...
export function ProcessPaymentDialog({ open, onOpenChange, payment, onSubmit }: ProcessPaymentDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const processData = {
                ...
                receiptNumber: formData.receiptNumber || `REC${new Date().getFullYear()}${String(Math.floor(Math.random() * 10000)).padStart(4, '0')}`,
    ...
    const handleInputChange = (field: string, value: string) => {
        ...
        if (field === "paymentMethod" && (value === "bank_transfer" || value === "card")) {
            setFormData(prev => ({
                ...prev,
                [field]: value,
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant={
                            payment.status === "pending" ? "secondary" :
                                payment.status === "partial" ? "outline" : "destructive"
                        }>
                            {payment.status}
                        </Badge>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-2">
                        ...
                        {isPartialPayment && paidAmount > 0 && (
                            <p className="text-sm text-orange-600">
                                Paiement partiel - Reste: {formatCurrency(remainingAmount - paidAmount)}
                            </p>
                        )}
                    </div>
                    ...
                    {(formData.paymentMethod === "bank_transfer" || formData.paymentMethod === "card") && (
                        <div className="space-y-2">
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                        ...
                        <div className="mt-2 space-y-1 text-sm">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-mono">{formatCurrency(Math.max(0, remainingAmount - paidAmount))}</span>
                            </div>
