...
export default function PaymentsContent() {
  ...
  const [paymentPlans, setPaymentPlans] =
    useState<PaymentPlan[]>(mockPaymentPlans);

  const [selectedPayment, setSelectedPayment] = useState<Payment | null>(null);
  const [selectedStudent, setSelectedStudent] =
    useState<StudentPaymentSummary | null>(null);
  const [selectedClass, setSelectedClass] =
    useState<ClassPaymentOverview | null>(null);
    ...
    const updatedPayments = payments.map((p) => {
      if (p.id === paymentId) {
        return {
          ...p,
          status:
            processData.paidAmount >= p.amount
              ? ("paid" as const)
              : ("partial" as const),
  ...
  const handleViewDetails = (
    item: Payment | StudentPaymentSummary | ClassPaymentOverview,
  ) => {
  ...
  const paymentsColumns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: Payment } }) => {
        ...
        const isDueSoon =
          (dueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24) <= 3;
  ...
  const studentColumns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: StudentPaymentSummary } }) => (
        <Badge
          variant={
            row.original.collectionRate >= 80 ? "default" : "destructive"
          }
        >
        ...
        <div className="flex items-center gap-2">
          ...
          {row.original.studentsWithOverdue > 0 && (
            <Badge variant="destructive" className="text-xs">
              {(
                (row.original.studentsWithOverdue /
