...
export function PaymentStats({
  ...
  // Type distribution
  const typeDistribution = payments.reduce(
    (acc, payment) => {
  ...
  const studentsWithOverdue = studentSummaries.filter(
    (s) => s.totalOverdue > 0,
  ).length;

  // Average payment delay
  const paidPayments = payments.filter(
    (p) => p.paidDate && p.status === "paid",
  );
  const averageDelay =
          ...
          <div className="space-y-2 md:space-y-3">
            {topClasses.map((classOverview, index) => (
              ...
              >
                ...
                <Badge
                  variant={
                    classOverview.collectionRate >= 90 ? "default" : "secondary"
                  }
