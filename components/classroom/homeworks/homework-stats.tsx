...
export function HomeworkStats({
  ...
  const submissionRate =
    totalSubmissions > 0 ? (submittedCount / totalSubmissions) * 100 : 0;
  const gradingRate =
    submittedCount > 0 ? (gradedCount / submittedCount) * 100 : 0;
  const onTimeRate =
    submittedCount > 0
      ? ((submittedCount - lateCount) / submittedCount) * 100
      : 0;
  ...
  const averageGrade =
    gradesWithValues.length > 0
      ? gradesWithValues.reduce((sum, s) => sum + s.grade!, 0) /
  ...
  const subjectDistribution = homeworks.reduce(
    (acc, homework) => {
  ...
  // Type distribution
  const typeDistribution = homeworks.reduce(
    (acc, homework) => {
  ...
  const priorityDistribution = homeworks.reduce(
    (acc, homework) => {
              ...
              .map(([priority, count]) => (
                ...
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {priority === "urgent" && (
                      <RiAlarmWarningLine className="h-3 w-3 md:h-4 md:w-4 text-red-600 shrink-0" />
                    )}
                    {priority === "high" && (
                      <RiStarLine className="h-3 w-3 md:h-4 md:w-4 text-orange-600 shrink-0" />
                    )}
                    {priority === "medium" && (
                      <RiTimeLine className="h-3 w-3 md:h-4 md:w-4 text-blue-600 shrink-0" />
                    )}
                    {priority === "low" && (
                      <RiCheckLine className="h-3 w-3 md:h-4 md:w-4 text-green-600 shrink-0" />
                    )}
                    <span className="text-xs md:text-sm capitalize truncate">
                      {priority}
                    </span>
                  </div>
                  <Badge
          ...
          <div className="space-y-2 md:space-y-3">
            {topClasses.map((classOverview, index) => (
              ...
              >
                ...
                <Badge
                  variant={
                    classOverview.averageSubmissionRate >= 90
                      ? "default"
                      : "secondary"
                  }
                  className="text-xs shrink-0 ml-2"
