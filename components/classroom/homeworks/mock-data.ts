...
const algerianTeacherNames = [
    "Prof. Benali Ahmed", "Prof. Khelifi Fatima", "Prof. Meziane Omar",
    "Prof. Boumediene Aicha", "Prof. Hamidi Youcef", "Prof. Cherif Samira",
    "Prof. Belkacem Omar", "Prof. Boukhalfa Karim", "Prof. Benabdallah Nadia",
    "Prof. Mammeri Rachid", "Prof. Boudjelal Leila", "Prof. Zidane Sofiane"
];
...
for (let i = -15; i < 45; i++) {
    ...
    const subject = algerianSubjects[Math.floor(Math.random() * algerianSubjects.length)];
    const classInfo = algerianClasses[Math.floor(Math.random() * algerianClasses.length)];
    const teacher = algerianTeacherNames[Math.floor(Math.random() * algerianTeacherNames.length)];
    const titles = homeworkTitles[subject as keyof typeof homeworkTitles] || ["Devoir général"];
    const title = titles[Math.floor(Math.random() * titles.length)];

    const types = ["assignment", "project", "reading", "exercise", "research"] as const;
    ...
    const estimatedDuration = type === "project" ?
        Math.floor(Math.random() * 300) + 180 : // 3-8 hours for projects
        Math.floor(Math.random() * 120) + 30;   // 30min-2.5hours for others

    const totalMarks = Math.random() > 0.3 ? Math.floor(Math.random() * 15) + 5 : undefined; // 5-20 marks
...
mockHomeworks.forEach(homework => {
    if (homework.status !== "draft") {
        ...
        for (let i = 0; i < numStudents; i++) {
            ...
            let status: "not_submitted" | "submitted" | "late" | "graded" | "returned";
            ...
            if (Math.random() > submissionRate) {
                ...
                if (isOnTime) {
                    ...
                    submitDate.setDate(submitDate.getDate() - Math.floor(Math.random() * 5)); // 0-5 days early
            ...
            mockHomeworkSubmissions.push({
                ...
                submittedAt,
                status,
                content: submittedAt ? `Réponse de ${studentName} pour ${homework.title}` : undefined,
                grade,
                feedback,
                gradedBy,
                gradedAt,
...
mockHomeworks.forEach(homework => {
    const submissions = mockHomeworkSubmissions.filter(s => s.homeworkId === homework.id);
    ...
    if (submissions.length > 0) {
        ...
        const averageGrade = grades.length > 0 ? grades.reduce((sum, grade) => sum + grade, 0) / grades.length : undefined;
...
const uniqueStudents = new Map<string, { id: string; name: string; classId: string; className: string }>();
...
uniqueStudents.forEach(student => {
    const studentSubmissions = mockHomeworkSubmissions.filter(s => s.studentId === student.id);
    ...
    if (studentSubmissions.length > 0) {
        ...
        const notSubmittedCount = studentSubmissions.filter(s => s.status === "not_submitted").length;
        ...
        const overdueHomeworks = studentHomeworks
            .filter(h => {
                ...
                return new Date(h.dueDate) < now && (!submission || submission.status === "not_submitted");
            })
            .slice(0, 5);
...
algerianClasses.forEach(classInfo => {
    const classHomeworks = mockHomeworks.filter(h => h.classId === classInfo.id);

    if (classHomeworks.length > 0) {
        const totalHomeworks = classHomeworks.length;
        const activeHomeworks = classHomeworks.filter(h => h.status === "assigned" || h.status === "in_progress").length;
        const completedHomeworks = classHomeworks.filter(h => h.status === "completed").length;
        const overdueHomeworks = classHomeworks.filter(h => h.status === "overdue").length;
        ...
        const averageSubmissionRate = classStats.length > 0 ?
            classStats.reduce((sum, stat) => sum + stat.submissionRate, 0) / classStats.length : 0;

        const gradesWithValues = classStats.filter(s => s.averageGrade !== undefined);
        const averageGrade = gradesWithValues.length > 0 ?
            gradesWithValues.reduce((sum, stat) => sum + stat.averageGrade!, 0) / gradesWithValues.length : undefined;
...
mockHomeworks.sort((a, b) => new Date(b.assignedDate).getTime() - new Date(a.assignedDate).getTime());
mockStudentHomeworkSummary.sort((a, b) => b.submissionRate - a.submissionRate);
mockClassHomeworkOverview.sort((a, b) => b.averageSubmissionRate - a.averageSubmissionRate);