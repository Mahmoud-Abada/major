...
export default function HomeworksContent() {
    ...
    const [studentSummaries, setStudentSummaries] = useState<StudentHomeworkSummary[]>(mockStudentHomeworkSummary);
    const [classOverviews, setClassOverviews] = useState<ClassHomeworkOverview[]>(mockClassHomeworkOverview);

    const [selectedHomework, setSelectedHomework] = useState<Homework | null>(null);
    const [selectedSubmission, setSelectedSubmission] = useState<HomeworkSubmission | null>(null);
    const [selectedStudent, setSelectedStudent] = useState<StudentHomeworkSummary | null>(null);
    ...
    const handleViewDetails = (item: Homework | HomeworkSubmission | StudentHomeworkSummary) => {
    ...
    const homeworksColumns = [
            ...
            cell: ({ row }: { row: { original: Homework } }) => (
                <Badge variant={
                    ...
                    <div className="flex items-center gap-1">
                        {row.original.priority === "urgent" && <RiAlarmWarningLine size={12} />}
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: Homework } }) => {
                ...
                const isDueSoon = (dueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24) <= 2;
                ...
                <Badge variant={
                    row.original.status === "completed" ? "default" :
                        row.original.status === "overdue" ? "destructive" :
                            row.original.status === "in_progress" ? "secondary" :
                                row.original.status === "assigned" ? "outline" : "secondary"
                }>
    ...
    const submissionsColumns = [
        {
            ...
            cell: ({ row }: { row: { original: HomeworkSubmission } }) => {
                const homework = homeworks.find(h => h.id === row.original.homeworkId);
                return (
                    <div>
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: HomeworkSubmission } }) => (
                <Badge variant={
                    row.original.status === "graded" ? "default" :
                        row.original.status === "submitted" ? "secondary" :
                            row.original.status === "late" ? "destructive" :
                                row.original.status === "returned" ? "outline" : "secondary"
                }>
                    <div className="flex items-center gap-1">
                        {row.original.status === "graded" && <RiCheckLine size={14} />}
                        {row.original.status === "not_submitted" && <RiCloseLine size={14} />}
    ...
    const studentColumns = [
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: StudentHomeworkSummary } }) => (
                <div className="flex items-center gap-2">
                    <span className="font-mono text-green-600">{row.original.submittedCount}</span>
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: StudentHomeworkSummary } }) => (
                <div className="flex items-center gap-2">
                    <span className="font-mono text-orange-600">{row.original.lateCount}</span>
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: StudentHomeworkSummary } }) => (
                <div className="flex items-center gap-2">
                    <span className="font-mono text-red-600">{row.original.notSubmittedCount}</span>
        ...
        },
        {
            ...
            header: "Rate",
            cell: ({ row }: { row: { original: StudentHomeworkSummary } }) => (
                <Badge variant={row.original.submissionRate >= 80 ? "default" : "destructive"}>
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: StudentHomeworkSummary } }) => (
                <span className="font-mono">
                    {row.original.averageGrade ? row.original.averageGrade.toFixed(1) : "-"}
                </span>
            ),
        },
        {
    ...
    const classColumns = [
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: ClassHomeworkOverview } }) => (
                <div className="flex items-center gap-2">
                    <span className="font-mono text-blue-600">{row.original.activeHomeworks}</span>
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: ClassHomeworkOverview } }) => (
                <div className="flex items-center gap-2">
                    <span className="font-mono text-green-600">{row.original.completedHomeworks}</span>
                    <RiCheckLine size={16} className="text-green-600" />
                </div>
            ),
        },
        {
            ...
            cell: ({ row }: { row: { original: ClassHomeworkOverview } }) => (
                <div className="flex items-center gap-2">
                    <span className="font-mono text-red-600">{row.original.overdueHomeworks}</span>
        ...
        },
        {
            ...
            cell: ({ row }: { row: { original: ClassHomeworkOverview } }) => (
                <Badge variant={row.original.averageSubmissionRate >= 80 ? "default" : "destructive"}>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-muted-foreground">Assign, track, and grade student homework assignments</p>
                    </div>
                    <div className="flex items-center gap-2">
