...
export function HomeworkDetailsDialog({
  ...
  const isHomework =
    "title" in data && "subject" in data && "assignedDate" in data;
  const isSubmission = "homeworkId" in data && "studentName" in data;
  const isStudentSummary =
    "recentSubmissions" in data && "upcomingHomeworks" in data;
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isSubmission &&
              `Submission - ${(data as HomeworkSubmission).studentName}`}
            {isStudentSummary &&
              `Student Summary - ${(data as StudentHomeworkSummary).studentName}`}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-6">
            ...
            <>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          (data as Homework).priority === "urgent"
                            ? "destructive"
                            : (data as Homework).priority === "high"
                              ? "default"
                              : (data as Homework).priority === "medium"
                                ? "secondary"
                                : "outline"
                        }
                        className="mt-1"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          (data as Homework).status === "completed"
                            ? "default"
                            : (data as Homework).status === "overdue"
                              ? "destructive"
                              : (data as Homework).status === "in_progress"
                                ? "secondary"
                                : (data as Homework).status === "assigned"
                                  ? "outline"
                                  : "secondary"
                        }
                        className="mt-1"
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  {(data as Homework).tags &&
                    (data as Homework).tags!.length > 0 && (
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          Tags
                        </p>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-medium mt-1">
                        {(data as HomeworkSubmission).studentName}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">
                      ...
                      <Badge
                        variant={
                          (data as HomeworkSubmission).status === "graded"
                            ? "default"
                            : (data as HomeworkSubmission).status ===
                                "submitted"
                              ? "secondary"
                              : (data as HomeworkSubmission).status === "late"
                                ? "destructive"
                                : (data as HomeworkSubmission).status ===
                                    "returned"
                                  ? "outline"
                      ...
                      >
                        <div className="flex items-center gap-1">
                          ...
                          {(data as HomeworkSubmission).status ===
                            "not_submitted" && <RiCloseLine size={14} />}
                          {(data as HomeworkSubmission).isLate && (
                            <RiTimeLine size={14} />
                          )}
                          {(data as HomeworkSubmission).status.replace(
                            "_",
                            " ",
                          )}
                        </div>
                      ...
                      <div>
                        ...
                        {(data as HomeworkSubmission).isLate ? (
                          <Badge variant="destructive" className="mt-1">
                            {(data as HomeworkSubmission).daysLate} day
                            {(data as HomeworkSubmission).daysLate !== 1
                              ? "s"
                              : ""}{" "}
                            late
                          </Badge>
                  ...
                  {(data as HomeworkSubmission).grade !== undefined && (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium mt-1">
                          {(data as HomeworkSubmission).gradedBy ||
                            "Not graded"}
                        </p>
                      </div>
                    </div>
          ...
          {isStudentSummary && (
            <>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-medium mt-1">
                        {(data as StudentHomeworkSummary).studentName}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">
                      ...
                      <p className="text-sm font-medium mt-1">
                        {(data as StudentHomeworkSummary).className}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-4 text-center">
                    <div className="p-3 bg-muted/50 rounded-lg">
                      <p className="text-lg font-bold">
                        {(data as StudentHomeworkSummary).totalHomeworks}
                      </p>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="font-medium">
                        {(
                          data as StudentHomeworkSummary
                        ).submissionRate.toFixed(1)}
                        %
                      </span>
                    </div>
                    <Progress
                      value={(data as StudentHomeworkSummary).submissionRate}
                      className="h-2"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Progress
                      value={(data as StudentHomeworkSummary).onTimeRate}
                      className="h-2"
                    />
                  </div>
                  ...
                  {(data as StudentHomeworkSummary).averageGrade && (
                    <div className="text-center">
                      <p className="text-lg font-bold">
                        {(data as StudentHomeworkSummary).averageGrade.toFixed(
                          1,
                        )}
                      </p>
              ...
              {(data as StudentHomeworkSummary).upcomingHomeworks.length >
                0 && (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {(data as StudentHomeworkSummary).upcomingHomeworks
                        .slice(0, 3)
                        .map((homework) => (
                          <div
                            key={homework.id}
              ...
              {(data as StudentHomeworkSummary).overdueHomeworks.length > 0 && (
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {(data as StudentHomeworkSummary).overdueHomeworks
                        .slice(0, 3)
                        .map((homework) => (
                          <div
                            key={homework.id}
