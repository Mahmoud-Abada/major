 ...
 * Allows adding multiple students to a group
 */
...
export default function AddStudentsToGroupDialog({
    open,
    onOpenChange,
    groupId,
    groupTitle,
    ...
    const handleAdd = async () => {
        ...
        try {
            const groupStudents = selectedStudents.map(studentId => ({
                groupId,
                studentId,
            }));
            ...
            if (result.status === "success") {
                toast.success(`Successfully added ${selectedStudents.length} student${selectedStudents.length > 1 ? 's' : ''} to ${groupTitle || 'group'}`);
                ...
                throw new Error(result.message || "Failed to add students to group");
            }

        } catch (error: any) {
            const errorMessage = error?.message || "Failed to add students to group";
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[80vh]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {groupTitle ? (
                            <>Select students to add to <strong>{groupTitle}</strong>. Students will become members of this study group and can participate in group activities.</>
                        ) : (
                            "Select students to add to the group. Students will become members of this study group and can participate in group activities."
                        )}
                    </DialogDescription>
                </DialogHeader>
                ...
                {selectedStudents.length > 0 && (
                    <div className="bg-muted/50 p-4 rounded-md">
                        <p className="text-sm text-muted-foreground">
                            ...
                            {groupTitle ? <strong>{groupTitle}</strong> : "the group"}.
                        </p>
                    </div>
                )}
