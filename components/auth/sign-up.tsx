...
import { personSchema, schoolSchema } from "@/lib/validationSchemas";
...
const UnifiedSignupForm = ({ userType }: { userType: UserType }) => {
  ...
  const steps = userType === "school" ? [1, 2, 3] : [1, 2];

  // Use appropriate schema based on user type
  const schema = userType === "school" ? schoolSchema : personSchema;
  ...
  const {
    ...
    mode: "onChange",
    defaultValues: {
      role: userType === "school" ? "school" : userType,
    },
  });
  ...
  const nextStep = async () => {
    ...
    } else if (currentStep === 2 && userType === "school") {
      isValid = await trigger([
        "representativeName",
        "representativeRole",
  ...
  const prepareSchoolData = (data: FormValues): RegisterData => {
    return {
      userData: {
        email: data.email,
        name: data.schoolName,
        password: data.password,
        username: data.email, // Using email as username for schools
        ...
        userType: "school",
        schoolType:
          data.schoolType === "other" ? data.customSchoolType : data.schoolType,
      },
      schoolForm: {
  ...
  const preparePersonData = (data: FormValues): RegisterData => {
    return {
      userData: {
        email: data.email,
        name: `${data.firstName} ${data.lastName}`,
        password: data.password,
        username: data.email, // Using email as username for persons
        phoneNumber: data.phone,
        location: locationData
          ? {
              fullLocation: locationData.fullLocation,
        ...
        userType: data.role as "student" | "teacher",
      },
    };
  };
