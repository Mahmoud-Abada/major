...
export class MockUserApiService {
  ...
  static async getUsers(filters?: {
    ...
    if (filters) {
      ...
      if (filters.search) {
        ...
        users = users.filter(
          (user) =>
            ...
            (user.role === "admin" &&
              (user as AdminUser).department
                ?.toLowerCase()
                .includes(searchLower)) ||
            (user.role === "teacher" &&
              (user as TeacherUser).subjects?.some((s) =>
      ...
      if (filters.department && filters.department !== "all") {
        users = users.filter(
          (user) =>
            user.role === "admin" &&
            (user as AdminUser).department === filters.department,
        );
      }
      ...
      if (filters.subject && filters.subject !== "all") {
        users = users.filter(
          (user) =>
            user.role === "teacher" &&
            (user as TeacherUser).subjects?.includes(filters.subject),
        );
      }
      ...
      if (filters.grade && filters.grade !== "all") {
        users = users.filter(
          (user) =>
            user.role === "student" &&
            (user as StudentUser).grade === filters.grade,
        );
      }
    }
    ...
    return {
      ...
      hasMore: filters?.limit
        ? (filters.offset || 0) + users.length < total
        : false,
    };
  }
  ...
  static async createUser(userData: Partial<User>): Promise<User> {
    ...
    const newUser: User = {
      ...
      ...(userData.role === "student" && {
        ...
        dateOfBirth: (userData as StudentUser).dateOfBirth
  ...
  static async updateUser(id: string, userData: Partial<User>): Promise<User> {
    ...
    if (
      userData.email &&
      users.some((user) => user.email === userData.email && user.id !== id)
    ) {
      throw new Error("Email already exists");
    }
  ...
  static async getUserStats(): Promise<{
    ...
    const recentlyCreated = users.filter(
      (user) => user.createdAt >= weekAgo,
    ).length;
    const activeToday = users.filter(
      (user) => user.lastLogin && user.lastLogin >= today,
    ).length;
...
export const AdminApiService = {
  ...
  createAdmin: (data: Partial<AdminUser>) =>
    MockUserApiService.createUser({ ...data, role: "admin" }),
  updateAdmin: (id: string, data: Partial<AdminUser>) =>
    MockUserApiService.updateUser(id, data),
...
export const TeacherApiService = {
  ...
  createTeacher: (data: Partial<TeacherUser>) =>
    MockUserApiService.createUser({ ...data, role: "teacher" }),
  updateTeacher: (id: string, data: Partial<TeacherUser>) =>
    MockUserApiService.updateUser(id, data),
...
export const StudentApiService = {
  ...
  createStudent: (data: Partial<StudentUser>) =>
    MockUserApiService.createUser({ ...data, role: "student" }),
  updateStudent: (id: string, data: Partial<StudentUser>) =>
    MockUserApiService.updateUser(id, data),
...
export const ParentApiService = {
  ...
  createParent: (data: Partial<ParentUser>) =>
    MockUserApiService.createUser({ ...data, role: "parent" }),
  updateParent: (id: string, data: Partial<ParentUser>) =>
    MockUserApiService.updateUser(id, data),
