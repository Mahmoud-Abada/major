...
import { selectTeacherById, updateTeacher } from "@/store/slices/classroom/teachersSlice";
...
export default function TeacherDetailPage() {
    ...
    const teacher = useSelector((state: any) => selectTeacherById(state, teacherId));
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">رقم الأستاذ: {teacher.teacherId}</p>
                    </div>
                </div>
                        ...
                        <div className="flex-1 space-y-4">
                                ...
                                <div className="flex items-center space-x-2">
                                    <Badge className={getStatusColor(teacher.status)}>
                                        {teacher.status === "Active" ? "نشط" :
                                            teacher.status === "Inactive" ? "غير نشط" : "في إجازة"}
                                    </Badge>
                                    {teacher.verified && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                                    ...
                                    <span>{teacher.wilaya}, {teacher.commune}</span>
                                </div>
                                <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                                    <Calendar className="h-4 w-4" />
                                    <span>انضم في {format(new Date(teacher.joinDate), "dd/MM/yyyy")}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">الخبرة المهنية</CardTitle>
                                <Award className="h-4 w-4 text-muted-foreground" />
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-3">
                                    <div className="text-2xl font-bold">{teacher.experience} سنة</div>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-3">
                                    <div className="text-2xl font-bold">{teacher.salary.toLocaleString()} دج</div>
                                    <p className="text-xs text-muted-foreground">
                                        {teacher.paymentSchedule === "Monthly" ? "شهرياً" :
                                            teacher.paymentSchedule === "Weekly" ? "أسبوعياً" : "لكل جلسة"}
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">الفصول المُدرَّسة</CardTitle>
                                <BookOpen className="h-4 w-4 text-muted-foreground" />
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-3">
                                    <div className="text-2xl font-bold">{teacher.classIds.length}</div>
                                    <p className="text-xs text-muted-foreground">
                                        فصل نشط
                                    </p>
                                </div>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm">{format(new Date(teacher.joinDate), "dd/MM/yyyy")}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">عدد المواد</label>
                                    <p className="text-sm">{teacher.subjectIds.length} مادة</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">عدد الطلاب</label>
                ...
                <TabsContent value="classes" className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                جميع الفصول التي يدرسها الأستاذ
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-3">
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-sm">{format(new Date(teacher.dateOfBirth), "dd/MM/yyyy")}</p>
                                </div>
                                <div>
