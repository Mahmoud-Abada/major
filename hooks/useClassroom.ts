...
export function useClassrooms(params: GetClassroomsParams, immediate = true) {
    return useApi(
        () => classroomApi.getClassrooms(params),
        { immediate }
    );
}
...
export function useCreateClassrooms() {
    return useMutation(
        (params: CreateClassroomParams) => classroomApi.createClassrooms(params)
    );
}

export function useUpdateClassroom() {
    return useMutation(
        (params: UpdateClassroomParams) => classroomApi.updateClassroom(params)
    );
}

export function useDeleteClassroom() {
    return useMutation(
        (classroomId: string) => classroomApi.deleteClassroom(classroomId)
    );
}
...
export function useUpdateGroup() {
    return useMutation(
        (params: { groupId: string; groupData: Partial<Group> }) =>
...
export function useDeleteGroup() {
    return useMutation(
        (groupId: string) => classroomApi.deleteGroup(groupId)
    );
}
...
export function useClassroomManager(initialParams: Partial<GetClassroomsParams> = {}) {
    ...
    const fetchClassrooms = useCallback(async (reset = false) => {
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            if (response.pagination.cursor) {
                setParams(prev => ({
                    ...prev,
                    pagination: { ...prev.pagination, cursor: response.pagination.cursor }
                }));
            }
        } catch (err: any) {
    ...
    const updateParams = useCallback((newParams: Partial<GetClassroomsParams>) => {
        setParams(prev => ({
            ...prev,
            ...newParams,
            pagination: { ...prev.pagination, cursor: undefined }
...
export function useClassroomOperations() {
    ...
    const createClassroom = useCallback(async (classroomData: Omit<Classroom, "id" | "createdAt" | "updatedAt">) => {
        return createMutation.mutate({ classrooms: [classroomData] });
    }, [createMutation]);

    const updateClassroom = useCallback(async (classroomId: string, classroomData: Partial<Classroom>) => {
    ...
    const archiveClassroom = useCallback(async (classroomId: string) => {
    ...
    const unarchiveClassroom = useCallback(async (classroomId: string) => {
        return updateMutation.mutate({
            classroomId,
...
export function useGroupOperations() {
    ...
    const createGroup = useCallback(async (groupData: Omit<Group, "id" | "createdAt" | "updatedAt">) => {
