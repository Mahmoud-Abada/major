...
export function useUpdatePost() {
    return useMutation(
        (params: { postId: string; postData: Partial<Post> }) =>
...
export function useDeletePost() {
    return useMutation(
        (postId: string) => classroomApi.deletePost(postId)
    );
}
...
export function useStudentPosts(studentId: string, immediate = true) {
    return useApi(
        () => classroomApi.getStudentPosts(studentId),
        { immediate }
    );
}
...
export function useGroupPosts(groupId: string, immediate = true) {
    return useApi(
        () => classroomApi.getGroupPosts(groupId),
        { immediate }
    );
}

export function useHomeworksAndQuizzes(params: {
    classroomId?: string;
    groupId?: string;
    studentId?: string;
}, immediate = true) {
    return useApi(
        () => classroomApi.getHomeworksAndQuizzes(params),
        { immediate }
    );
}
...
export function useInteractWithPost() {
    return useMutation(
        (params: { postId: string; userId: string; interaction: "like" | "dislike" }) =>
...
export function usePostsManager(initialParams: {
    classroomId?: string;
    groupId?: string;
    type?: string;
    studentId?: string;
    ...
    const fetchPosts = useCallback(async () => {
        ...
        try {
            ...
            if (params.studentId) {
                ...
                throw new Error('Either classroomId, groupId, or studentId is required');
            }
...
export function usePostOperations() {
    ...
    const dislikePost = useCallback(async (postId: string, userId: string) => {
        return interactMutation.mutate({ postId, userId, interaction: 'dislike' });
    }, [interactMutation]);

    const submitHomework = useCallback(async (postId: string, studentId: string, submission: any) => {
        return submitHomeworkMutation.mutate({ postId, studentId, submission });
    }, [submitHomeworkMutation]);

    const submitQuiz = useCallback(async (postId: string, studentId: string, answers: any[]) => {
        return submitQuizMutation.mutate({ postId, studentId, answers });
    }, [submitQuizMutation]);

    const submitPoll = useCallback(async (postId: string, studentId: string, choice: string) => {
    ...
    const unpublishPost = useCallback(async (postId: string) => {
        return updateMutation.mutate({ postId, postData: { isPublished: false } });
    }, [updateMutation]);
    ...
    return {
        ...
        loading: createMutation.loading || updateMutation.loading || deleteMutation.loading ||
            interactMutation.loading || submitHomeworkMutation.loading ||
            submitQuizMutation.loading || submitPollMutation.loading,
        error: createMutation.error || updateMutation.error || deleteMutation.error ||
            interactMutation.error || submitHomeworkMutation.error ||
            submitQuizMutation.error || submitPollMutation.error
    };
}
...
export function usePostStatistics(posts: Post[]) {
    ...
    useEffect(() => {
        ...
        const recentActivity = posts.filter(post => post.createdAt > weekAgo).length;
