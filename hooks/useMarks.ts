...
export function useCreateMark() {
  return useMutation((mark: Omit<Mark, "id" | "createdAt" | "updatedAt">) =>
    classroomApi.createMark(mark),
  );
}

export function useUpdateMark() {
  return useMutation((params: { markId: string; markData: Partial<Mark> }) =>
    classroomApi.updateMark(params),
  );
}

export function useDeleteMark() {
  return useMutation((markId: string) => classroomApi.deleteMark(markId));
}
...
export function useStudentMarks(
  ...
  return useApi(() => classroomApi.getStudentMarks(params), { immediate });
}
...
export function useGroupMarks(groupId: string, immediate = true) {
  return useApi(() => classroomApi.getGroupMarks(groupId), { immediate });
}
...
export function useMarksManager(
  initialParams: {
    studentId?: string;
    classroomId?: string;
    groupId?: string;
    ...
    try {
      ...
      if (params.markType) {
        filteredMarks = filteredMarks.filter(
          (mark) => mark.markType === params.markType,
        );
      }
...
export function useMarkOperations() {
  ...
  const createMark = useCallback(
    async (markData: Omit<Mark, "id" | "createdAt" | "updatedAt">) => {
  ...
  const updateMark = useCallback(
    async (markId: string, markData: Partial<Mark>) => {
  ...
  const updateStudentMark = useCallback(
    async (studentId: string, markId: string, markData: Partial<Mark>) => {
      return updateStudentMutation.mutate({ studentId, markId, markData });
    },
    [updateStudentMutation],
  );

  const updateGroupMark = useCallback(
    async (groupId: string, markId: string, markData: Partial<Mark>) => {
  ...
  const updateExemption = useCallback(
    async (groupId: string, studentId: string, isExempted: boolean) => {
  ...
  return {
    ...
    loading:
      ...
      updateStudentMutation.loading ||
      updateGroupMutation.loading ||
      updateExemptionMutation.loading,
    error:
...
export function useMarkStatistics(marks: Mark[]) {
  ...
  useEffect(() => {
    ...
    const passingRate =
      (scores.filter((score) => score >= 50).length / scores.length) * 100;
    ...
    Object.keys(bySubject).forEach((subject) => {
      bySubject[subject].average =
        bySubject[subject].average / bySubject[subject].count;
    });
    ...
    if (recentMarks.length > 0 && olderMarks.length > 0) {
      const recentAvg =
        recentMarks.reduce(
          (sum, mark) => sum + (mark.value / mark.maxValue) * 100,
          0,
        ) / recentMarks.length;
      const olderAvg =
        olderMarks.reduce(
