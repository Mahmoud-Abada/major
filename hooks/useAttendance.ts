...
export function useStudentAttendance(
  ...
  return useApi(() => classroomApi.getStudentAttendance(params), { immediate });
}

export function useEventAttendance(eventId: string, immediate = true) {
  return useApi(() => classroomApi.getEventAttendance(eventId), { immediate });
}
...
export function useClassroomAttendance(
  ...
  return useApi(() => classroomApi.getClassroomAttendance(params), {
    immediate,
  });
}
  ...
  const fetchAttendance = useCallback(async () => {
    ...
    try {
      ...
      if (params.eventId) {
        ...
        throw new Error(
          "Either studentId, classroomId, or eventId is required",
        );
      }
...
export function useAttendanceOperations() {
  ...
  const updateAttendance = useCallback(
    async (attendanceId: string, attendanceData: Partial<Attendance>) => {
  ...
  const markAbsent = useCallback(
    async (attendanceId: string, notes?: string) => {
      return updateMutation.mutate({
        attendanceId,
...
export function useAttendanceStatistics(attendance: Attendance[]) {
  ...
  useEffect(() => {
    ...
    Object.keys(byStudent).forEach((student) => {
      ...
      studentData.rate =
        total > 0
          ? ((studentData.present + studentData.late + studentData.excused) /
              total) *
            100
          : 0;
    });
    ...
    const recentAttendance = sortedAttendance.slice(
      0,
      Math.ceil(sortedAttendance.length / 3),
    );
    const olderAttendance = sortedAttendance.slice(
      Math.ceil(sortedAttendance.length / 3),
    );
...
export function useBulkAttendance() {
  ...
  const updateBulkAttendance = useCallback(
    async (
      updates: Array<{
        attendanceId: string;
        status: "present" | "absent" | "late" | "excused";
