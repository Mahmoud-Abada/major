...
export class FilesApiService {
    ...
    async uploadFile(
        ...
        if (options?.metadata) additionalData.metadata = JSON.stringify(options.metadata);
    ...
    async downloadFile(fileId: string): Promise<Blob> {
        const response = await apiClient.get<Blob>(`${this.baseUrl}/${fileId}/download`);
        return response;
    }
    ...
    async searchFiles(query: string, filters?: FileFilters): Promise<FileUpload[]> {
        const response = await apiClient.get<FileUpload[]>(`${this.baseUrl}/search`, {
            q: query,
            ...filters,
        });
        return response;
    ...
    async getFolders(parent?: string): Promise<Array<{
        id: string;
        name: string;
        path: string;
        parent?: string;
    ...
    async deleteFolder(folderId: string, recursive = false): Promise<void> {
        await apiClient.delete(`${this.baseUrl}/folders/${folderId}`, { recursive });
    }
