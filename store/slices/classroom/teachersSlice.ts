...
const teachersSlice = createSlice({
    ...
    reducers: {
        ...
        updateTeacher: (state, action: PayloadAction<{ id: string; data: Partial<TeacherFormData> }>) => {
        ...
        deleteMultipleTeachers: (state, action: PayloadAction<string[]>) => {
            state.teachers = state.teachers.filter(teacher => !action.payload.includes(teacher.id));
            state.pagination.total -= action.payload.length;
            if (state.selectedTeacher && action.payload.includes(state.selectedTeacher.id)) {
                state.selectedTeacher = null;
            }
        },
        ...
        // Filters
        setFilters: (state, action: PayloadAction<Partial<TeachersState['filters']>>) => {
        ...
        // Pagination
        setPagination: (state, action: PayloadAction<Partial<TeachersState['pagination']>>) => {
            state.pagination = { ...state.pagination, ...action.payload };
        },

        // Bulk operations
        updateTeacherStatus: (state, action: PayloadAction<{ ids: string[]; status: Teacher['status'] }>) => {
        ...
        assignTeacherToClass: (state, action: PayloadAction<{ teacherId: string; classId: string }>) => {
        ...
        removeTeacherFromClass: (state, action: PayloadAction<{ teacherId: string; classId: string }>) => {
            const { teacherId, classId } = action.payload;
            const teacher = state.teachers.find(t => t.id === teacherId);
            if (teacher) {
        ...
        // Payment operations
        addPaymentToTeacher: (state, action: PayloadAction<{ teacherId: string; payment: any }>) => {
...
export const selectSelectedTeacher = (state: RootState) => state.teachers.selectedTeacher;
...
export const selectTeachersPagination = (state: RootState) => state.teachers.pagination;
...
export const selectFilteredTeachers = (state: RootState) => {
    ...
    return teachers.filter(teacher => {
        ...
        if (filters.experience.length > 0) {
            ...
            if (experienceRange === "6-10" && (teacher.experience < 6 || teacher.experience > 10)) return false;
            if (experienceRange === "11-20" && (teacher.experience < 11 || teacher.experience > 20)) return false;
        ...
        if (filters.search) {
            ...
            return (
                ...
                teacher.specialization.some(spec => spec.toLowerCase().includes(searchLower))
            );
        }
...
export const selectTeachersBySpecialization = (state: RootState, specialization: string) =>
