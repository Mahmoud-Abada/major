...
const studentsSlice = createSlice({
  ...
  reducers: {
    ...
    updateStudent: (
      state,
      action: PayloadAction<{ id: string; data: Partial<StudentFormData> }>,
    ) => {
    ...
    deleteMultipleStudents: (state, action: PayloadAction<string[]>) => {
      ...
      if (
        ...
        action.payload.includes(state.selectedStudent.id)
      ) {
        state.selectedStudent = null;
      }
    },
    ...
    setPagination: (
      state,
      action: PayloadAction<Partial<StudentsState["pagination"]>>,
    ) => {
    ...
    updateStudentStatus: (
      state,
      action: PayloadAction<{ ids: string[]; status: Student["status"] }>,
    ) => {
    ...
    addPaymentToStudent: (
      state,
      action: PayloadAction<{ studentId: string; payment: any }>,
    ) => {
...
export const selectStudentsFilters = (state: RootState) =>
  state.students.filters;
export const selectStudentsPagination = (state: RootState) =>
...
export const selectStudentsByClass = (state: RootState, classId: string) =>
  state.students.students.filter((student) =>
