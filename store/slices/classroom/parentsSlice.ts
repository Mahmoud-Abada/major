...
const parentsSlice = createSlice({
    ...
    reducers: {
        ...
        updateParent: (state, action: PayloadAction<{ id: string; data: Partial<ParentFormData> }>) => {
        ...
        deleteMultipleParents: (state, action: PayloadAction<string[]>) => {
            state.parents = state.parents.filter(parent => !action.payload.includes(parent.id));
            state.pagination.total -= action.payload.length;
            if (state.selectedParent && action.payload.includes(state.selectedParent.id)) {
                state.selectedParent = null;
            }
        },
        ...
        // Filters
        setFilters: (state, action: PayloadAction<Partial<ParentsState['filters']>>) => {
        ...
        // Bulk operations
        updateParentStatus: (state, action: PayloadAction<{ ids: string[]; status: Parent['status'] }>) => {
        ...
        addStudentToParent: (state, action: PayloadAction<{ parentId: string; studentId: string }>) => {
        ...
        removeStudentFromParent: (state, action: PayloadAction<{ parentId: string; studentId: string }>) => {
            const { parentId, studentId } = action.payload;
            const parent = state.parents.find(p => p.id === parentId);
            if (parent) {
        ...
        // Payment operations
        addPaymentToParent: (state, action: PayloadAction<{ parentId: string; payment: any }>) => {
...
export const selectSelectedParent = (state: RootState) => state.parents.selectedParent;
...
export const selectParentsPagination = (state: RootState) => state.parents.pagination;
...
export const selectFilteredParents = (state: RootState) => {
    ...
    return parents.filter(parent => {
        ...
        // Relationship filter
        if (filters.relationship.length > 0 && !filters.relationship.includes(parent.relationship)) {
            return false;
        }
...
export const selectParentsByStudent = (state: RootState, studentId: string) =>
    state.parents.parents.filter(parent => parent.studentIds.includes(studentId));
