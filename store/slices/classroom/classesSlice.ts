...
const classesSlice = createSlice({
    ...
    reducers: {
        ...
        updateClass: (state, action: PayloadAction<{ id: string; data: Partial<Class> }>) => {
        ...
        addStudentToClass: (state, action: PayloadAction<{ classId: string; studentId: string }>) => {
        ...
        removeStudentFromClass: (state, action: PayloadAction<{ classId: string; studentId: string }>) => {
            ...
            if (classIndex !== -1) {
                const currentClass = state.classes[classIndex];
                currentClass.studentIds = currentClass.studentIds.filter(id => id !== action.payload.studentId);
                currentClass.currentStudents = currentClass.studentIds.length;
                currentClass.updatedAt = new Date();
            }
        },
...
// Selectors
export const selectAllClasses = (state: { classes: ClassesState }) => state.classes.classes;
export const selectClassById = (state: { classes: ClassesState }, id: string) =>
    state.classes.classes.find(c => c.id === id);
export const selectClassesByTeacher = (state: { classes: ClassesState }, teacherId: string) =>
    state.classes.classes.filter(c => c.teacherId === teacherId);
export const selectClassesByGrade = (state: { classes: ClassesState }, grade: string) =>
