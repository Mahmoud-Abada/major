...
const groupsSlice = createSlice({
    ...
    reducers: {
        ...
        updateGroup: (state, action: PayloadAction<{ id: string; data: Partial<Group> }>) => {
        ...
        addMemberToGroup: (state, action: PayloadAction<{ groupId: string; memberId: string }>) => {
            ...
            if (groupIndex !== -1) {
                ...
                if (!currentGroup.memberIds.includes(action.payload.memberId) &&
        ...
        removeMemberFromGroup: (state, action: PayloadAction<{ groupId: string; memberId: string }>) => {
            ...
            if (groupIndex !== -1) {
                const currentGroup = state.groups[groupIndex];
                currentGroup.memberIds = currentGroup.memberIds.filter(id => id !== action.payload.memberId);
                currentGroup.currentMembers = currentGroup.memberIds.length;
                currentGroup.updatedAt = new Date();
            }
        },
...
export const selectGroupsByType = (state: { groups: GroupsState }, type: string) =>
    state.groups.groups.filter(g => g.type === type);
export const selectGroupsByCreator = (state: { groups: GroupsState }, creatorId: string) =>
